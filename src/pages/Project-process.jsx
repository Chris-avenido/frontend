                    ...
                    >
                        ...
                        <div className="fund-modal-body app-scroll">
                            ...
                            <section className="fund-modal-project-card">
                                ...
                                <p>{project.school_name || 'Mariwaya Elementary School'} | {project.school_id || '123456'}</p>
                            </section>
                                        ...
                                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-4 flex items-center gap-3 text-base font-bold text-[var(--ink-soft)]">
                                                    ...
                                                    <span className="truncate">{project.region || 'No Region'} | {project.division || 'No Division'}</span>
                                                </div>
                                                <p className="mt-3 pl-7 text-sm font-medium text-[var(--muted)]">
                                                    {project.school_name || 'No School'} {project.school_id ? `| ${project.school_id}` : ''}
                                                </p>
                                            </div>
                                            <span className={`w-fit shrink-0 rounded-lg border px-4 py-1.5 text-sm font-bold ${getStatusStyle(project.status_of_construction_phase)}`}>
                                                {project.status_of_construction_phase || 'Active'}
                                            </span>
                                        </div>

                                        <div className="mt-5 border-t border-[var(--line-soft)] pt-4">
                                            <div className="flex items-center gap-3 text-base font-extrabold text-[var(--ink)]">
                                                <DollarSign className="h-4 w-4 text-[var(--brand-gold)]" />
                                                <span>{formatCurrency(project.approved_budget_for_contract)}</span>
                                            </div>
                                        </div>
                                    </motion.article>
                                ))}
